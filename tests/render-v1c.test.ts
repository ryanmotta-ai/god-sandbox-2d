...
const input = { camera, tileMap: map, entities: [entity], cities: new Map([[city.id, city]]), kingdoms: new Map(), overlayMode: 'none' as const, selection: { x: 10, y: 10, radius: 1, color: '#fbbf24' }, viewportWidth: 480, viewportHeight: 320, devicePixelRatio: 1 } as any;
const first = builder.build(input);
assert.ok(first.staticInstances > 0, 'building is emitted into its resident chunk');
assert.ok(first.dynamicInstances >= 2, 'citizen and selection are batched dynamically');
...
assert.ok(steady.dynamicInstances >= 2, 'animated instances remain visible without per-entity draw calls');
