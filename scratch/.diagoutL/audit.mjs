...
var Household = class Household {
	...
	/** Spends from the purse, never dipping below the reserve. Returns coin spent. */
	spend(amount) {
		const spendable = Math.max(0, this.coin - 6);
...
var SimulationEngine = class {
	...
	*/
	...
	householdShop(household) {
		...
		const paid = household.spend(cost);
		if (paid <= 0) return;
		const bought = Math.min(wanted, paid / unitPrice);
		...
		if (kingdom) kingdom.treasury.add("gold", paid * .25);
	}
	/**
