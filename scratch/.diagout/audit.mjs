...
var WorldMarket = class {
	...
	reset() {
		this.entries.clear();
		for (const good of ALL_GOODS) this.entries.set(good, {
			price: GOODS[good].basePrice,
			supply: 0,
			demand: 0,
			history: [GOODS[good].basePrice]
		});
	}
	price(good) {
...
var CivilizationEngine = class {
	...
	produceGoods(city, world, multiplier) {
		...
		const foraged = city.stock.add("food", city.population * .12);
	...
	assignVisibleWorker(worker, building) {
		...
		} else if (type === "workshop" || type === "smithy" || type === "factory" || type === "refinery") worker.profession = "builder";
	...
	*/
	...
	expandTerritory(city, world, bonus) {
		...
		const claimsPerYear = 1 + Math.floor(city.population / 25);
		for (let i = 0; i < claimsPerYear && city.territory.size < limit; i++) {
			...
			for (const key of city.territory) {
				...
				for (const [dx, dy] of [
					...
					const good = tileResourceToGood(tile.resourceType);
					let score = 5 - Math.hypot(tile.x - city.x, tile.y - city.y) * .08;
					if (good) {
						score += Math.min(30, tile.resourceAmount / 10);
						score += GOODS[good].tier === "strategic" ? 80 : GOODS[good].tier === "regional" ? 28 : 10;
					}
					if (world.tileMap.isCoastalLand(tile.x, tile.y)) score += 8;
					if (tile.type === TerrainType.MOUNTAIN && !good && city.species !== SpeciesType.STONEKIN) score -= 30;
					frontier.set(tKey, {
						x: tile.x,
						y: tile.y,
						score
			...
			const chosen = choices[rng.chance(.8) ? 0 : Math.min(choices.length - 1, 1)];
	...
	*/
	tickColonisation(world) {
		for (const city of [...world.cities.values()]) {
			...
			if (!kingdom) continue;
			if (city.population < 22) continue;
			if (city.prosperity < .5) continue;
			if (city.stock.get("food") < 120) continue;
			if (city.population < city.housingCapacity() * .85) continue;
			const chance = .1 * (GOVERNMENTS[kingdom.government].expansion / 20);
...
var SimulationEngine = class {
	...
	tickAI(tileMap, particles) {
		...
		for (const e of this.entities) {
			...
			if (distMoved > .001) {
				...
				if (currentTile && !TERRAINS[currentTile.type].isWater && currentTile.type !== TerrainType.MOUNTAIN) {
					if (Math.random() < .25) {
						currentTile.roadTraffic++;
						if (currentTile.roadTraffic >= 5 && currentTile.roadLevel === 0) currentTile.roadLevel = 1;
						else if (currentTile.roadTraffic >= 25 && currentTile.roadLevel === 1) currentTile.roadLevel = 2;
						else if (currentTile.roadTraffic >= 75 && currentTile.roadLevel === 2) currentTile.roadLevel = 3;
					}
				}
				if (isHumanoid) {
			...
			if (distMoved < .005 && e.aiState !== "idle" && e.aiState !== "heal" && e.aiState !== "sleep") {
				...
				if (e.stuckTicks >= 2) {
					e.stuckTicks = 0;
					const jittered = SimplePathfinder.jitterAround(e.x, e.y, tileMap, SPECIES_DEFINITIONS[e.species].baseSpeed * .025);
					if (jittered) {
						e.x = jittered.x;
						e.y = jittered.y;
						e._jitterFailures++;
		...
		const dayTicks = this.yearTickCounter % 1440;
		const currentHour = Math.floor(dayTicks / 60);
	...
	get24HourTime() {
		const dayTicks = this.yearTickCounter % 1440;
		const hour = Math.floor(dayTicks / 60);
		const minute = dayTicks % 60;
	...
	getCalendarDate() {
		const dayInYear = Math.floor(this.yearTickCounter % 34560 / 1440);
		return {
			month: Math.min(12, Math.floor(dayInYear / 2) + 1),
			day: dayInYear % 2 + 1,
			year: this.currentYear
		};
	}
	tickFauna(e, tileMap, particles) {
		const speed = SPECIES_DEFINITIONS[e.species].baseSpeed * .025;
		switch (e.species) {
	...
	executeHumanoidState(e, tileMap, particles, speed) {
		switch (e.aiState) {
			...
			case "gather_wood": {
				...
				if (e.targetX === null || e.targetY === null) {
					const target = gCity.nearestCached(TerrainType.FOREST, e.x, e.y) ?? this.findNearestTileType(e.x, e.y, TerrainType.FOREST, tileMap, 12);
					if (target) {
			...
			case "gather_food": {
				...
				const fx = bld ? bld.x + .5 : gCity.x;
				const fy = bld ? bld.y + .5 : gCity.y;
	...
	tickFamilies(tileMap) {
		...
		for (const [cityId, residents] of byCity) {
			...
			for (const parent of residents) {
				...
				let chance = .35 * city.prosperity + Math.min(.25, foodPerHead / 25);
	...
	*/
	...
	processBirthEvent(mother) {
		...
		mother.fertilityCooldown = rng.rangeInt(3, 6);
...
function generateDeposits(grid, width, height, seed) {
	...
	for (let x = 0; x < width; x++) for (let y = 0; y < height; y++) {
		...
		if (tile.type === TerrainType.MOUNTAIN && rng.chance(.5)) {
