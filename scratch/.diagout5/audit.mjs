...
var SimulationEngine = class {
	...
	tickAI(tileMap, particles) {
		...
		const currentHour = Math.floor(this.yearTickCounter % 300 / 300 * 24);
	...
	get24HourTime() {
		const dayFraction = this.yearTickCounter % 300 / 300;
	...
	getCalendarDate() {
		const dayInYear = Math.floor(this.yearTickCounter % TICKS_PER_YEAR / 300);
		return {
			month: Math.min(12, Math.floor(dayInYear / (24 / 12)) + 1),
			day: dayInYear % (24 / 12) + 1,
			year: this.currentYear
		};
	}
