			...
			for (let y = 0; y < h; y++) {
				...
				if (tile.isOnFire) {
					...
					if (x > 0) {
						...
						if (!n.isOnFire && config.flammability > .05 && rng.chance(config.flammability * .035 * moisturePenalty)) fireQueue.push(n);
					}
					if (x < w - 1) {
						const n = g[x + 1][y];
						const config = TERRAINS[n.type];
...
function factionTargets(id, current, ctx) {
	...
	switch (id) {
		...
		case "merchants":
			...
			satisfaction += note("Volume de comércio", ctx.tradeDependency * .3) + ctx.culture.openness * .14 + ctx.stability * .12 + note("Guerra fecha rotas", -warPain * .22) + note("Impostos", -taxPain * .12) + note("Embargos", -clamp01$1(ctx.embargoes * .35) * .28);
...
var SimplePathfinder = class {
	...
	*/
	static fleeFrom(myX, myY, threatX, threatY, tileMap, speed = .2) {
		...
		const dist = Math.sqrt(dx * dx + dy * dy);
		if (dist < .01) return this.getStepTowards(myX, myY, myX + rng.range(-2.5, 2.5), myY + rng.range(-2.5, 2.5), tileMap, speed);
...
var GreatPersonManager = class {
	...
	static checkAscension(entities, kingdoms, cities, tileMap, year) {
		...
		for (const e of entities) {
			...
			if (!kingdom) continue;
			let type = null;
			if (e.level >= 4 && e.profession === "scout" && rng.chance(.12)) type = "scholar";
			else if (e.kills >= 3 || e.profession === "king" && kingdom.militaryPower > 150 && rng.chance(.1)) type = "hero";
			else if (e.level >= 3 && (e.profession === "woodcutter" || e.profession === "miner") && rng.chance(.1)) type = "builder";
			else if (e.level >= 4 && kingdom.knownKingdoms.size >= 2 && rng.chance(.08)) type = "diplomat";
			if (type) {
	...
	*/
	tickColonisation(world) {
		for (const city of [...world.cities.values()]) {
			...
			const colony = new City(nextId("city"), this.generateSettlementName(city, world), city.species, site.x, site.y, city.founderName, world.year);
...
var SimulationEngine = class {
	...
	spawnEntity(species, x, y, tileMap, forcedGender) {
		...
		const entity = new Entity(nextId("ent"), species, spawnX, spawnY);
				...
				if (target) {
					...
					if (targetDist <= maxReach && e.attackCooldown <= 0) {
						...
						if (e.species === SpeciesType.EMBERKIN) {
							...
							if (tile && rng.chance(.05)) tile.isOnFire = true;
						}
	...
	*/
	...
	processBirthEvent(mother) {
		...
		for (let b = 0; b < birthCount; b++) {
			const { child } = conceiveChild(fatherEntity, mother, birthX + b * .3, birthY, () => nextId("ent"));
			child.showEmote("👶", 60);
...
//#endregion
//#region scratch/test_determinism.ts
/**
...
function run() {
	...
	for (let i = 0; i < 12; i++) sim.spawnEntity(fauna[i % fauna.length], rng.rangeInt(2, 77), rng.rangeInt(2, 77), map);
	sim.totalBirths = 0;
	sim.totalDeaths = 0;
	...
	return {
		...
		entities: sim.entities.map((e) => `${e.id}|${e.species}|${e.x.toFixed(4)},${e.y.toFixed(4)}|hp${e.hp.toFixed(2)}|h${e.needs.hunger.toFixed(2)}|${e.profession}`).sort(),
		cities: [...sim.cities.values()].map((c) => `${c.id}|${c.name}|pop${c.population}|terr${c.territory.size}|b${c.buildings.size}|` + ALL_GOODS.map((g) => c.stock.get(g).toFixed(2)).join(",")).sort(),
...
check("the same seed produces the same number of entities", a.entities.length === b.entities.length, `${a.entities.length} vs ${b.entities.length}`);
check("the same seed produces the same number of settlements", a.cities.length === b.cities.length, `${a.cities.length} vs ${b.cities.length}`);
var firstEntityDiff = "";
for (let i = 0; i < Math.min(a.entities.length, b.entities.length); i++) if (a.entities[i] !== b.entities[i]) {
	firstEntityDiff = `\n      A: ${a.entities[i]}\n      B: ${b.entities[i]}`;
	break;
}
check("every citizen is in the same place with the same state", firstEntityDiff === "", firstEntityDiff);
var firstCityDiff = "";
for (let i = 0; i < Math.min(a.cities.length, b.cities.length); i++) if (a.cities[i] !== b.cities[i]) {
	firstCityDiff = `\n      A: ${a.cities[i]}\n      B: ${b.cities[i]}`;
	break;
}
check("every settlement holds exactly the same goods", firstCityDiff === "", firstCityDiff);
var differentSummary = (() => {
	rng.setSeed(2654435934);
	resetIds();
	const map = new TileMap(SIZE, SIZE, "single_continent", 999);
	const sim = new SimulationEngine();
	const particles = new ParticleManager();
	for (let k = 0; k < 8; k++) sim.spawnEntity(SpeciesType.LUMINI, 20 + k % 3, 20 + Math.floor(k / 3), map, k % 2 === 0 ? "male" : "female");
	for (let t = 0; t < TICKS_PER_YEAR * YEARS; t++) sim.tickAI(map, particles);
	return `${sim.entities.length}|${sim.cities.size}`;
})();
check("a different seed produces a different world", differentSummary !== `${a.entities.length}|${a.cities.length}`, `both ${differentSummary}`);
