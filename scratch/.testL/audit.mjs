...
//#endregion
//#region scratch/test_balance.ts
/**
...
console.log("=== PHASE L: BALANCE, PERFORMANCE, REMAINING TESTS ===\n");
{
	...
	check("a smithy with no coal forges no steel", city.stock.get("steel") === 0, `steel=${city.stock.get("steel")}`);
	check("and the iron it could not use is still sitting there", city.stock.get("iron") > 0, `iron=${city.stock.get("iron")}`);
	city.stock.set("coal", 200);
	for (let t = 0; t < TICKS_PER_YEAR; t++) sim.tickAI(map, particles);
	check("supplying the missing input restarts production", city.stock.get("steel") > 0, `steel=${city.stock.get("steel")}`);
}
{
	const trade = new TradeNetwork();
	trade.signAgreement("kA", "kB", 1, .1);
	...
	check("an embargoed pair cannot sign a new agreement", trade.signAgreement("kA", "kB", 3, .1) === null);
	trade.liftEmbargo("kA", "kB");
	check("lifting the embargo lets them deal again", trade.signAgreement("kA", "kB", 4, .1) !== null);
...
}
{
	...
	kingdom.cityIds.add("r1");
	kingdom.cityIds.add("r2");
	forceLand(map, [{
		x: 5,
		y: 5
	}, {
	...
	const world = {
		year: 1,
		cities: /* @__PURE__ */ new Map([["r1", mine], ["r2", forge]]),
		kingdoms: /* @__PURE__ */ new Map([["kr", kingdom]]),
		tileMap: map,
	...
	mine.stock.set("coal", 500);
	forge.stock.set("coal", 0);
	...
	map.getTile(5, 5).railDamage = .6;
	map.getTile(6, 5).railDamage = .6;
	mine.stock.set("coal", 500);
	forge.stock.set("coal", 0);
	rail.tickFreight(world);
...
}
{
	...
	for (let i = 0; i < 26; i++) sim.spawnEntity(fauna[i % fauna.length], place.rangeInt(2, 125), place.rangeInt(2, 125), map);
	...
	for (let t = 0; t < TICKS_PER_YEAR + 1; t++) {
		...
		if (sim.currentYear !== startYear) {
			yearTickMs = cost;
			break;
		}
	}
	console.log(`\n  world: ${entities} entities (${humans} humanoid), ${sim.cities.size} cities, ${sim.kingdoms.size} realms`);
	console.log(`  ordinary tick: ${msPerTick.toFixed(4)} ms  (${Math.round(1e3 / msPerTick)} ticks/s)`);
	console.log(`  year-boundary tick: ${yearTickMs} ms (whole civilisation layer)`);
	console.log(`  a year is ${TICKS_PER_YEAR} ticks of 600/day\n`);
	check("the sim keeps ahead of the fastest game speed", 1e3 / msPerTick > 600, `${Math.round(1e3 / msPerTick)} ticks/s vs 600 needed`);
	check("a year boundary does not stall the frame past a blink", yearTickMs < 250, `${yearTickMs} ms`);
	check("the world is actually populated for this measurement", humans > 10 && sim.cities.size > 0, `${humans} humanoids, ${sim.cities.size} cities`);
}
