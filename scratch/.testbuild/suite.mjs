...
function factionTargets(id, current, ctx) {
	...
	switch (id) {
		case "peasants":
			...
			satisfaction += note("Segurança alimentar", ctx.foodSecurity * .28, {
				kind: "good",
				good: "food"
			}) + note("Prosperidade", ctx.prosperity * .16) + note("Impostos", -taxPain * .22) + note("Guerra", -warPain * .12) + note("Preço do grão que vendem", foodPricePain * .08, {
				kind: "good",
				good: "food"
			});
		...
		case "merchants":
			...
			satisfaction += note("Volume de comércio", ctx.tradeDependency * .3, { kind: "trade" }) + ctx.culture.openness * .14 + ctx.stability * .12 + note("Guerra fecha rotas", -warPain * .22, { kind: "trade" }) + note("Impostos", -taxPain * .12) + note("Embargos", -clamp01(ctx.embargoes * .35) * .28, { kind: "trade" });
		...
		case "workers":
			...
			satisfaction += ctx.industrialisation * .08 + ctx.foodSecurity * .12 + note("Desigualdade", -inequalityPain * .26) + note("Impostos", -taxPain * .08) + note("Preço da comida", -foodPricePain * .3, {
				kind: "good",
				good: "food"
			}) + note("Desemprego", -joblessPain * .32, { kind: "jobs" });
...
}
{
	...
	check("the food-price pressure points at food itself", workerFood?.source?.kind === "good" && workerFood.source.good === "food", JSON.stringify(workerFood?.source));
	const jobs = (strained.factions.workers.factors ?? []).find((f) => f.label.includes("Desemprego"));
	check("the unemployment pressure points at the jobs data", jobs?.source?.kind === "jobs", JSON.stringify(jobs?.source));
	const embargo = (strained.factions.merchants.factors ?? []).find((f) => f.label.includes("Embargo"));
	check("the embargo pressure points at trade", embargo?.source?.kind === "trade", JSON.stringify(embargo?.source));
	...
	check("every good a pressure points at is a real good", bogus.length === 0, bogus.map((f) => f.label).join(", "));
	check("pressures with no evidence to show carry no source", allFactors.some((f) => !f.source), "todo fator declarou uma fonte, o que provavelmente é engano");
}
{
	let bad = 0;
