	...
	*/
	...
	openTradeRoutes(world) {
		for (const agreement of world.trade.agreements.values()) {
			...
			if (kind === "overland") this.paveTradeRoad(fromCity, toCity, world);
	...
	findTradePairing(a, b, world) {
		...
		let bestGain = 0;
		const tariff = world.trade.getAgreement(a.id, b.id)?.tariff ?? .12;
		for (const [source, destination, seller, buyer] of [[
			...
			for (const good of ALL_GOODS) {
				...
				const transport = worldPrice * distance * .004;
	...
	*/
	...
	updateCaravans(routes, cities, kingdoms, tileMap, particles, currentYear) {
		...
		for (const route of routes.values()) {
			...
			const roadMultiplier = ROAD_SPEED_BONUS[currentTile?.roadLevel ?? 0] ?? 1;
...
function run(label, wildlife, seed) {
	...
	if (wildlife) {
		...
		const count = Math.round(9216 / 700);
		const place = new RandomService(seed ^ 2654435769);
		for (let i = 0; i < count; i++) sim.spawnEntity(fauna[i % fauna.length], place.rangeInt(2, 93), place.rangeInt(2, 93), map);
	}
	sim.totalBirths = 0;
	sim.totalDeaths = 0;
	const isHum = (s) => SPECIES_DEFINITIONS[s].isHumanoid;
