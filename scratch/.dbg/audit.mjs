...
//#endregion
//#region scratch/debug_economy.ts
/**
...
*/
var YEARS = Math.min(10, Number(process.env.YEARS ?? 8));
var SIZE = Number(process.env.SIZE ?? 100);
var SEED = Number(process.env.SEED ?? 2024);
var pad = (v, n) => String(v).padEnd(n);
var num = (n, d = 1) => (Math.round(n * 10 ** d) / 10 ** d).toString();
var map = new TileMap(SIZE, SIZE, "single_continent", SEED);
var sim = new SimulationEngine();
var particles = new ParticleManager();
var place = new RandomService(SEED);
var species = [
	SpeciesType.LUMINI,
	SpeciesType.SYLVANII,
	SpeciesType.STONEKIN
...
for (let i = 0; i < Math.round(SIZE * SIZE / 700); i++) sim.spawnEntity(fauna[i % fauna.length], place.rangeInt(2, SIZE - 3), place.rangeInt(2, SIZE - 3), map);
sim.totalBirths = 0;
...
for (const c of sim.cities.values()) {
	...
	console.log(`  ${pad(c.name, 16)}${pad(c.population, 5)}${pad(workers, 8)}${pad(`${filled}/${jobs}`, 6)}${pad(`${num(unemp, 0)}%`, 7)}${pad(num(f.produced), 9)}${pad(num(f.consumed), 9)}${pad(num(c.ledger.net("food")), 8)}${num(c.stock.get("food"), 0)}`);
}
console.log("\n--- GOODS (world) ---");
console.log(`  ${pad("good", 12)}${pad("price", 8)}${pad("supply", 9)}${pad("demand", 9)}${pad("produced", 10)}${pad("consumed", 10)}stock`);
for (const g of ALL_GOODS) {
	...
	console.log(`  ${pad(g, 12)}${pad(num(sim.market.price(g), 2), 8)}${pad(num(sim.market.supplyOf(g)), 9)}${pad(num(sim.market.demandOf(g)), 9)}${pad(num(produced), 10)}${pad(num(consumed), 10)}${num(stock, 0)}`);
}
console.log("\n--- TRADE ---");
if (sim.trade.routes.size === 0) console.log("  (no routes — a route needs surplus on one side, scarcity on the other, and a margin that survives haulage and tariff)");
else for (const r of sim.trade.routes.values()) {
	const from = sim.cities.get(r.fromCityId);
	const to = sim.cities.get(r.toCityId);
	const capacity = r.kind === "maritime" ? from && to ? portCapacityFactor(from, to) : 0 : roadCapacityFactor(r.path, map);
	console.log(`  ${pad(GOODS[r.good].name, 12)} ${pad(from?.name ?? "?", 14)}→ ${pad(to?.name ?? "?", 14)}vol=${pad(num(r.volume, 0), 5)}cap=${pad(`${num(capacity * 100, 0)}%`, 6)}${r.active ? "active" : "SUSPENDED"}`);
}
...
for (const k of sim.kingdoms.values()) {
	...
	for (const g of ALL_GOODS) {
		...
		if (used > 0 && imp / used > .2) deps.push(`${g} ${num(imp / used * 100, 0)}%`);
	}
	console.log(`  ${k.name}`);
	...
	console.log(`     factions: ${[
		...
		"merchants",
		"military"
	].map((f) => `${f} ${num(k.society.factions[f].satisfaction * 100, 0)}%`).join("  ")}`);
	console.log(`     revoltRisk=${num(k.society.revoltRisk * 100, 0)}%  reformPressure=${num(k.society.reformPressure * 100, 0)}%`);
}
var humans = sim.entities.filter((e) => SPECIES_DEFINITIONS[e.species].isHumanoid);
var wild = sim.entities.length - humans.length;
var starving = humans.filter((e) => e.needs.hunger >= 88).length;
console.log("\n--- POPULATION ---");
