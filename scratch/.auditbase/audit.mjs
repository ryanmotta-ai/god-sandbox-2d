		...
		for (const [cityId, residents] of byCity) {
			...
			for (const parent of residents) {
				...
				if (parent.species === SpeciesType.STONEKIN) {
					...
					city.ledger.recordConsumed("stone", city.stock.take("stone", 8));
					chance *= .85;
...
console.log("\n--- ROADS (after 8 years) ---");
{
	...
	for (let x = 0; x < tileMap.width; x++) for (let y = 0; y < tileMap.height; y++) roadCount[tileMap.getTile(x, y)?.roadLevel ?? 0]++;
