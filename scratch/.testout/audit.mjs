...
//#endregion
//#region scratch/test_economy.ts
/**
...
}
{
	...
	check("price cannot run away", p > .01 && p < 1e5, `oil = ${p}`);
}
{
	const market = new WorldMarket();
...
}
{
	...
	for (const c of cities) for (const g of ALL_GOODS) {
	...
	check("food is not consumed twice over", foodConsumed <= ceiling || totalPop === 0, `eaten=${foodConsumed.toFixed(1)} pop=${totalPop} ceiling=${ceiling}`);
...
}
{
	...
	const rich = new LocalMarket();
	const barren = new LocalMarket();
	for (let y = 0; y < 25; y++) {
		rich.settle("iron", world.price("iron"), 900, 40);
		barren.settle("iron", world.price("iron"), 5, 300);
	}
	...
	check("local prices stay anchored to the world price", dear < world.price("iron") * 4 && cheap > world.price("iron") * .2);
}
{
	const ledger = new CityLedger();
	...
	check("import dependency is computed, not invented", Math.abs(dep - .86) < .01, `got ${(dep * 100).toFixed(0)}%`);
...
}
{
	...
	check("dear food angers the townsfolk", dearBread.factions.workers.satisfaction < calm.factions.workers.satisfaction, `${calm.factions.workers.satisfaction.toFixed(2)} -> ${dearBread.factions.workers.satisfaction.toFixed(2)}`);
	check("dear food does NOT anger the countryside the same way", dearBread.factions.peasants.satisfaction >= calm.factions.peasants.satisfaction, `${calm.factions.peasants.satisfaction.toFixed(2)} -> ${dearBread.factions.peasants.satisfaction.toFixed(2)}`);
	check("unemployment radicalises the workforce", jobless.factions.workers.radicalization > calm.factions.workers.radicalization, `${calm.factions.workers.radicalization.toFixed(2)} -> ${jobless.factions.workers.radicalization.toFixed(2)}`);
	check("embargo hurts the merchants specifically", blockaded.factions.merchants.satisfaction < calm.factions.merchants.satisfaction, `${calm.factions.merchants.satisfaction.toFixed(2)} -> ${blockaded.factions.merchants.satisfaction.toFixed(2)}`);
	const crisis = settle({
		...baseCtx,
		foodPriceIndex: 2.6,
		unemployment: .3
	});
	check("hunger plus joblessness raises revolt risk", crisis.revoltRisk > calm.revoltRisk, `${calm.revoltRisk.toFixed(2)} -> ${crisis.revoltRisk.toFixed(2)}`);
	check("and it raises the pressure to reform", crisis.reformPressure > calm.reformPressure, `${calm.reformPressure.toFixed(2)} -> ${crisis.reformPressure.toFixed(2)}`);
}
{
	...
	open.culture.mercantilism = .5;
	closed.culture.mercantilism = .5;
	...
	check("free trade lowers the border tariff", openRate < .12, `${(openRate * 100).toFixed(0)}%`);
	check("closed markets raise the border tariff", closedRate > .2, `${(closedRate * 100).toFixed(0)}%`);
	check("a trade law moves tariffs more than culture does", closedRate > openRate * 2, `open ${(openRate * 100).toFixed(0)}% vs closed ${(closedRate * 100).toFixed(0)}%`);
	check("tariffs stay inside sane bounds", openRate >= .02 && closedRate <= .45);
	const buyPrice = 14;
	const marginOpen = 3 - buyPrice * openRate;
	const marginClosed = 3 - buyPrice * closedRate;
	check("protectionism can make a profitable route unprofitable", marginOpen > 0 && marginClosed < marginOpen, `open margin ${marginOpen.toFixed(2)}, closed ${marginClosed.toFixed(2)}`);
}
