...
var SimulationEngine = class {
	...
	*/
	...
	eatFromTheLand(e, needs) {
		...
		for (let dx = -3; dx <= 3; dx++) for (let dy = -3; dy <= 3; dy++) {
			...
			needs.hunger = Math.max(0, needs.hunger - 55);
			e.starvingDays = 0;
			e.showEmote("🍇", 30);
			return;
		}
	...
	*/
	...
	householdShop(household) {
		...
		const unitPrice = Math.max(.4, this.market.price("food") * .35 * scarcity);
		...
		const daysFood = household.size * 1.5;
...
*/
var SIZE = Number(process.env.SIZE ?? 128);
var SEED = Number(process.env.SEED ?? 777);
var WARMUP_TICKS = Number(process.env.WARMUP ?? 2e3);
var MEASURE_TICKS = Number(process.env.TICKS ?? 4e3);
/** Also run the full-map fire/fluid pass every 2 ticks + diplomacy every 10, like main.ts does. */
var WITH_FULL_LOOP = process.env.FULL === "1";
...
var after = snapshot();
var elapsedMs = Number(t1 - t0) / 1e6;
var ticksPerSec = MEASURE_TICKS / elapsedMs * 1e3;
var msPerTick = elapsedMs / MEASURE_TICKS;
