...
*/
function strategicWeight(research, good) {
	let weight = 0;
...
function strategicGoodsFor(research) {
	const totals = /* @__PURE__ */ new Map();
	for (const techId of research.known) {
		const demand = TECH_STRATEGIC_DEMAND[techId];
		if (!demand) continue;
		for (const [good, weight] of Object.entries(demand)) totals.set(good, (totals.get(good) ?? 0) + weight);
	}
	return [...totals.entries()].map(([good, weight]) => ({
		good,
		weight
	})).sort((a, b) => b.weight - a.weight);
}
/** Whether any realm in the world has a technological reason to want this good yet. */
function isStrategicNow(researchStates, good) {
	for (const research of researchStates) if (strategicWeight(research, good) > 0) return true;
	return false;
}
/**
...
*/
function technologyCapacity(research, hasBuilding, canObtain) {
	const out = [];
	for (const techId of research.known) {
		...
		const demand = TECH_STRATEGIC_DEMAND[techId] ?? {};
		const goods = Object.keys(demand);
...
*/
function operatingEra(research, capabilities) {
	...
	for (const techId of research.known) {
		...
		if (capacity !== void 0 && capacity < .5) continue;
...
function factionTargets(id, current, ctx) {
	...
	const foodPricePain = clamp01$1((ctx.foodPriceIndex - 1) / 1.2);
	const joblessPain = clamp01$1(ctx.unemployment / .3);
	...
	switch (id) {
		case "peasants":
			...
			satisfaction += ctx.foodSecurity * .28 + ctx.prosperity * .16 - taxPain * .22 - warPain * .12 + foodPricePain * .08;
		...
		case "merchants":
			...
			satisfaction += ctx.tradeDependency * .3 + ctx.culture.openness * .14 + ctx.stability * .12 - warPain * .22 - taxPain * .12 - clamp01$1(ctx.embargoes * .35) * .28;
			wealth = .35 + ctx.tradeDependency * .34 + (ctx.economy === "market" ? .18 : 0) - warPain * .12 - clamp01$1(ctx.embargoes * .35) * .2;
		...
		case "workers":
			...
			satisfaction += ctx.industrialisation * .08 + ctx.foodSecurity * .12 - inequalityPain * .26 - taxPain * .08 - foodPricePain * .3 - joblessPain * .32;
...
var DiplomacyManager = class {
	...
	recordBattle(k1, k2, k1Kills, k2Kills) {
		...
		if (war) {
			...
			if (k1Kills > 0) this.changeRelation(k1, k2, -k1Kills * .4);
			if (k2Kills > 0) this.changeRelation(k2, k1, -k2Kills * .4);
		}
	}
	endWar(k1, k2, year) {
		const key = this.getPairKey(k1, k2);
	...
	createAlliance(k1, k2, name, year) {
		...
		if (a1 && a2 && a1.id === a2.id) return null;
		let alliance;
		if (a1 && a2) {
	...
	allianceOf(kingdomId) {
		for (const a of this.alliances.values()) if (a.members.has(kingdomId)) return a;
		return null;
	}
	...
	tickDiplomacy(kingdomIds, year) {
		...
		for (let i = 0; i < ids.length; i++) for (let j = i + 1; j < ids.length; j++) {
			...
			const k2 = ids[j];
			const rel = this.getRelation(k1, k2);
			if (rel > 5) this.changeRelation(k1, k2, -.75);
...
*/
function damageRailAround(tileMap, cx, cy, radius) {
	for (let dx = -radius; dx <= radius; dx++) for (let dy = -radius; dy <= radius; dy++) {
		if (Math.hypot(dx, dy) > radius) continue;
		const tile = tileMap.getTile(Math.floor(cx) + dx, Math.floor(cy) + dy);
		if (!tile || tile.railLevel <= 0) continue;
		tile.railDamage = Math.min(1, tile.railDamage + rng.range(.18, .42));
	}
}
/**
...
*/
function damageStrategicBuildings(city) {
	const vital = [...city.buildings.values()].filter((b) => STRATEGIC_BUILDINGS.includes(b.type)).sort((a, b) => STRATEGIC_BUILDINGS.indexOf(a.type) - STRATEGIC_BUILDINGS.indexOf(b.type));
	...
	for (let i = 0; i < hits; i++) {
		...
		target.hp = Math.max(1, Math.round(target.hp * rng.range(.72, .95)));
	}
}
...
*/
function damagePrimaryRoads(tileMap, cx, cy, radius) {
	let best = null;
	let bestScore = -1;
	for (let dx = -radius; dx <= radius; dx++) for (let dy = -radius; dy <= radius; dy++) {
		const tile = tileMap.getTile(Math.floor(cx) + dx, Math.floor(cy) + dy);
	...
	if (best) best.roadDamage = Math.min(1, best.roadDamage + .6);
}
//#endregion
...
var CivilizationEngine = class {
	...
	collapsedRoutes = /* @__PURE__ */ new Set();
	reset() {
	...
	produceGoods(city, world, multiplier) {
		...
		for (const building of staffed) {
			...
			if (def.category !== "craft" && def.consumes) {
				...
				for (const [goodKey, amountValue] of Object.entries(def.consumes)) {
					const used = city.stock.take(goodKey, amountValue * Math.max(.25, building.outputMultiplier()));
					city.ledger.recordConsumed(goodKey, used);
				}
			}
	...
	runCraftProduction(city, world, kingdom, building, capacity) {
		...
		while (remaining > .05 && safety++ < 8) {
			...
			for (const [inputKey, qtyValue] of Object.entries(best.recipe.inputs)) {
				const used = city.stock.take(inputKey, qtyValue * cycles);
				city.ledger.recordConsumed(inputKey, used);
			}
	...
	*/
	...
	constructBuilding(city, world, kingdom) {
		...
		for (const [good, amount] of Object.entries(def.cost)) city.ledger.recordConsumed(good, amount);
	...
	*/
	paveRoadBetween(fromCity, toX, toY, world) {
		...
		const level = !!(kingdom?.research.knows("roads") || kingdom?.research.knows("masonry")) ? 2 : 1;
		for (const step of path) {
	...
	*/
	paveTradeRoad(fromCity, toCity, world) {
		...
		const level = !!(fromKingdom?.research.knows("roads") || fromKingdom?.research.knows("masonry")) ? 2 : 1;
		for (const step of path) {
			const tile = world.tileMap.getTile(Math.floor(step.x), Math.floor(step.y));
			if (!tile) continue;
			if (tile.roadLevel < level) tile.roadLevel = level;
	...
	repairCityInfrastructure(city, world) {
	...
	scoreBuilding(type, city, kingdom, world, resourceGood, siteScore) {
		...
		if (def.defense) {
			let atWar = false;
			if (kingdom) for (const other of world.kingdoms.values()) {
				if (other.id === kingdom.id) continue;
				if (world.diplomacy.isAtWar(other.id, kingdom.id)) {
					atWar = true;
					break;
				}
			}
			const threat = kingdom?.externalThreat ?? 0;
			const militarism = kingdom?.culture.militarism ?? .3;
			score += (def.defense - 1) * (atWar ? 180 : 20 + threat * 160 + militarism * 60);
		}
		if (type === "barracks") {
			const soldierJobs = city.countOfType("barracks") * 4;
			const target = Math.max(2, city.population * .05);
	...
	tryUpgradeBuilding(city) {
		...
		if (city.stock.spend(cost)) {
			for (const [good, amount] of Object.entries(cost)) city.ledger.recordConsumed(good, amount);
			target.upgrade();
		}
	}
	...
	collectTaxes(kingdom, world) {
		...
		const tradeIncome = kingdom.economy.hasCurrency ? kingdom.economy.fromWorldValue(yearlyTradeGold) : yearlyTradeGold;
	...
	*/
	chooseTech(kingdom, world) {
		...
		const hasCoastalCity = [...kingdom.cityIds].some((id) => {
			const c = world.cities.get(id);
			if (!c) return false;
			for (const b of c.buildings.values()) if (b.type === "port" || b.type === "harbor") return true;
			return c.buildingsOfCategory("commerce").some((b) => {
				return world.tileMap.getNeighbors(b.x, b.y, true).some((n) => TERRAINS[n.type].isWater);
			});
		});
		...
		for (const tech of available) {
			...
			if (idleTechnology > .35 && tech.track === "craft") score -= idleTechnology * 18;
	...
	*/
	reportTechnologicalDemand(kingdom, world) {
		const industrialBase = Math.max(1, kingdom.totalPopulation * .05 + kingdom.economy.industrialisation * kingdom.totalPopulation * .15);
		for (const { good, weight } of strategicGoodsFor(kingdom.research)) {
			const wanted = weight * industrialBase * .1;
	...
	*/
	assessTechnologicalCapacity(kingdom, world) {
		const cities = [...kingdom.cityIds].map((id) => world.cities.get(id)).filter((c) => !!c);
		const hasBuilding = (type) => cities.some((c) => c.hasBuilding(type));
		const canObtain = (good) => {
	...
	*/
	repriceLocalMarket(kingdom, world) {
		const cities = [...kingdom.cityIds].map((id) => world.cities.get(id)).filter((c) => !!c);
	...
	*/
	economicPressures(kingdom, world) {
		const cities = [...kingdom.cityIds].map((id) => world.cities.get(id)).filter((c) => !!c);
		const foodPriceIndex = kingdom.economy.market.price("food", world.market.price("food")) / Math.max(.01, GOODS.food.basePrice);
		let jobs = 0;
		let filled = 0;
		for (const city of cities) {
			jobs += city.jobCount();
		...
		return {
			foodPriceIndex,
			unemployment: workers > 0 ? clamp((workers - filled) / workers, 0, 1) : 0,
			labourShortage: jobs > 0 ? clamp((jobs - filled) / jobs, 0, 1) : 0,
			embargoes: world.trade.embargoes.filter((e) => e.againstKingdom === kingdom.id).length
		};
	}
	...
	tickStrategicDiplomacy(world) {
		...
		for (let i = 0; i < kingdoms.length; i++) for (let j = i + 1; j < kingdoms.length; j++) {
			...
			world.diplomacy.getRelation(a.id, b.id);
			...
			drift += sameSpecies ? .45 : -.45 + avgOpenness * .2;
			drift += sameEconomy ? .18 : 0;
			drift += ideologicalConflict ? -.9 : 0;
			drift += (affinity - .5) * 1;
			drift += avgTrust > .58 ? .15 : -Math.max(0, .45 - avgTrust) * .5;
			drift += tradeAgreement ? .4 + avgMercantilism * .2 : 0;
			drift += commonEnemy ? .65 : 0;
			drift += Math.max(0, avgSocialPeace - .52) * .42;
			drift -= proximity * (govA.aggression + govB.aggression) * .34;
			drift -= proximity * borderAmbition * .26;
			drift -= proximity * Math.max(0, avgSocialWar - .46) * .3;
			...
			if (world.trade.isEmbargoed(a.id, b.id)) finalDrift -= 1;
			if (Math.abs(finalDrift) >= .15) world.diplomacy.changeRelation(a.id, b.id, finalDrift);
			const newRelation = world.diplomacy.getRelation(a.id, b.id);
			if (a.research.knowsFeature("diplomacy_pacts") && b.research.knowsFeature("diplomacy_pacts") && !alreadyAllied && !truce && newRelation >= 62) {
	...
	negotiateTradeAgreements(world) {
		...
		for (let i = 0; i < kingdoms.length; i++) for (let j = i + 1; j < kingdoms.length; j++) {
			...
			if (relation >= 0 && rng.chance(.35)) {
				const tariff = Math.max(.01, (a.tariffRate() + b.tariffRate()) / 2 - Math.min(.09, relation / 1200));
	...
	openTradeRoutes(world) {
		for (const agreement of world.trade.agreements.values()) {
			...
			const routePath = kind === "overland" ? this.paveTradeRoad(fromCity, toCity, world) : void 0;
	...
	findTradePairing(a, b, world) {
		...
		for (const [source, destination, seller, buyer] of [[
			...
			b,
			a
		]]) for (const from of source) for (const to of destination) {
			...
			for (const good of ALL_GOODS) {
				...
				const transport = kind === "maritime" ? worldPrice * distance * .003 : worldPrice * distance * .004 * (1.5 - .3 * avgRoad);
	...
	runTradeRoutes(world) {
		for (const route of world.trade.routes.values()) {
			...
			const capacity = route.kind === "maritime" ? portCapacityFactor(from, to) : roadCapacityFactor(route.path, world.tileMap);
			if (capacity <= .01) {
				if (!this.collapsedRoutes.has(route.id)) {
					this.collapsedRoutes.add(route.id);
					chronicle.log(world.year, "disaster", `Maritime trade through ${GOODS[route.good].name} route ${from.name}–${to.name} collapsed: the harbor at ${portOperational(from) ? to.name : from.name} lies in ruins.`, {
	...
	*/
	determineRouteKind(from, to, tileMap, landPath) {
		const landPathComputed = landPath ?? SimplePathfinder.findPath(from.x, from.y, to.x, to.y, tileMap, "land");
		const landDist = landPathComputed.length > 1 ? landPathComputed.reduce((sum, _, i, arr) => i > 0 ? sum + Math.hypot(arr[i].x - arr[i - 1].x, arr[i].y - arr[i - 1].y) : 0, 0) : Infinity;
	...
	findCityPortWaterTile(city, tileMap) {
		const facilities = [...city.buildings.values()].filter((b) => (b.type === "port" || b.type === "harbor") && b.hp / b.maxHp > .5).sort((a, b) => a.type === "port" ? -1 : 1);
...
*/
var PANTRY_TARGET_PER_MEMBER = 1.2;
var Household = class Household {
	id;
	cityId;
	homeBuildingId;
	...
	pantry = new Stockpile(60);
	foundedYear;
	/** Running record so the UI can show a family getting richer or poorer. */
	lastEarned = 0;
	lastSpent = 0;
	constructor(id, cityId, homeBuildingId, foundedYear) {
		this.id = id;
		this.cityId = cityId;
	...
	*/
	spend(amount, essential = false) {
		const floor = essential ? 0 : 6;
	...
	static deserialize(data) {
		...
		h.coin = data.coin ?? 0;
		h.pantry.deserialize(data.pantry);
		return h;
	}
};
...
var WarfareSystem = class {
	...
	*/
	...
	pressSiege(city, owner, assessment, world) {
		...
		city.ledger.recordConsumed("food", city.stock.take("food", city.population * .3));
...
var NavalSystem = class NavalSystem {
	...
	*/
	...
	dockShip(ship, cityName, dockingKingdomId, counterpartyKingdomId, exporterKingdomId, kingdoms, particles) {
		const dockingKingdom = kingdoms.get(dockingKingdomId);
		kingdoms.get(exporterKingdomId);
		kingdoms.get(counterpartyKingdomId);
	...
	*/
	findPortWaterTile(city, tileMap) {
		...
		const waterBodySize = (sx, sy) => {
			const visited = /* @__PURE__ */ new Set();
			const queue = [[sx, sy]];
			visited.add(`${sx},${sy}`);
			let count = 0;
			while (queue.length > 0 && count < 5e3) {
		...
		for (const facility of facilities) {
			...
			for (const w of water) if (waterBodySize(w.x, w.y) >= 20) return {
				x: w.x + .5,
				y: w.y + .5
			};
...
var CaravanSystem = class {
	...
	*/
	...
	updateCaravans(routes, cities, kingdoms, tileMap, particles, currentYear) {
		...
		for (const route of routes.values()) {
			...
			const roadMultiplier = ROAD_SPEED_BONUS[currentTile?.roadLevelEffective ?? 0] ?? 1;
			const terrainMultiplier = currentTile ? TERRAIN_AFFINITY[caravan.caravanType][currentTile.type] ?? 1 : 1;
	...
	*/
	settleCaravanTrade(caravan, sellerKingdom, buyerKingdom, route, tileMap, particles) {
		this.paveRouteLeg(caravan, sellerKingdom, tileMap);
		if (route.good) particles.spawnDamageNumber(caravan.x, caravan.y, Math.round(caravan.cargoAmount));
	}
	/**
	...
	*/
	paveRouteLeg(caravan, kingdom, tileMap) {
		...
		for (const step of caravan.path) {
			...
			if (tile.roadLevel === 0 && tile.roadTraffic >= ROAD_UPGRADE_THRESHOLDS.dirt) tile.roadLevel = 1;
			else if (tile.roadLevel === 1 && tile.roadTraffic >= ROAD_UPGRADE_THRESHOLDS.stone && hasRoadsTech) tile.roadLevel = 2;
			else if (tile.roadLevel === 2 && tile.roadTraffic >= ROAD_UPGRADE_THRESHOLDS.imperial && hasEngineering) tile.roadLevel = 3;
		}
	}
};
//#endregion
...
var SimulationEngine = class {
	...
	tickAI(tileMap, particles) {
		...
		for (const e of this.entities) {
			...
			if (distMoved < .005 && e.aiState !== "idle" && e.aiState !== "heal") {
		...
		if (this.yearTickCounter % 600 === 0) this.tickDay();
	...
	*/
	tickDay() {
		...
		for (const e of this.entities) {
			...
			const needs = e.needs;
			const appetite = e.isChild ? .65 : 1;
			needs.hunger = Math.min(100, needs.hunger + 16 * appetite);
			if (household && (e.isChild || needs.hunger >= 88)) {
				const portion = e.isChild ? MEAL_CHILD : MEAL_ADULT;
				if (household.takeMeal(portion)) {
					needs.hunger = Math.max(0, needs.hunger - 62);
					e.starvingDays = 0;
				}
			} else if (!household && needs.hunger >= 55) this.eatFromTheLand(e, needs);
			const home = this.homeBuildingOf(e);
			const comfortTarget = home ? (home.type === "house" ? 72 : 58) + (home.level - 1) * 8 : 22;
			needs.comfort += (comfortTarget - needs.comfort) * .3;
			const threats = this.spatialHash.queryRadius(e.x, e.y, 7).filter((o) => this.isEnemy(e, o) || this.isFaunaThreat(o)).length;
			const safetyTarget = Math.max(5, 88 - threats * 22);
			needs.safety += (safetyTarget - needs.safety) * .35;
			if (needs.hunger >= 88) {
				e.starvingDays++;
				if (e.starvingDays > 2) e.hp -= 1.5 + e.starvingDays * .6;
				e.showEmote("💀", 60);
			} else e.starvingDays = 0;
		}
	}
	/** A person with no household forages where they stand. Needs the tileMap cached. */
	eatFromTheLand(e, needs) {
		...
		for (let dx = -3; dx <= 3; dx++) for (let dy = -3; dy <= 3; dy++) {
			...
			needs.hunger = Math.max(0, needs.hunger - 62 * .9);
			e.starvingDays = 0;
			e.showEmote("🍇", 30);
			return;
		}
	...
	*/
	...
	householdShop(household) {
		...
		const foodPrice = kingdomForPrice ? kingdomForPrice.economy.market.price("food", this.market.price("food")) : this.market.price("food");
		const scarcity = Math.max(.25, Math.min(3, city.population * 6 / Math.max(1, available)));
		const unitPrice = Math.max(.4, foodPrice * .35 * scarcity);
		...
		if (kingdom && paid > 0) kingdom.treasury.add("gold", paid * .25);
	}
	/**
	...
	*/
	pickUpLoad(e, tileMap, tx, ty, expected) {
	...
	* anything at all.
	*/
	preyRisk(target, nearby, tileMap) {
		...
		if (tileMap.getTile(Math.floor(target.x), Math.floor(target.y))?.kingdomId) risk += 14;
		return risk;
	}
	...
	decideHumanoidState(e, tileMap, particles) {
		...
		if (e.cityId && !e.isChild && e.needs.hunger >= 55 && e.aiState !== "eat") {
	...
	*/
	...
	assignProfession(e, city) {
		...
		for (const b of city.buildings.values()) if (b.type === "barracks") barracksFilled += b.assignedWorkerIds.size;
		const fed = city.stock.get("food") >= city.population * 2;
		const soldierTarget = Math.max(2, city.population * .05);
		...
		for (const building of city.buildings.values()) {
			...
			const score = jobs * building.level + (def.category === "food" ? 1e3 : def.category === "extraction" ? 800 : def.category === "craft" ? 600 : 400) + (building.type === "barracks" ? soldierBoost : 0);
			if (!best || score > best.score) best = {
				building,
				score
			};
	...
	*/
	claimHome(e, city) {
		...
		if (!e.householdId) {
			...
			} else e.householdId = `hh_${home.id}_${e.id}`;
		}
		const household = this.householdFor(e);
		if (household) {
			household.memberIds.add(e.id);
		...
		switch (e.aiState) {
			...
			case "eat": {
				...
				if (household && household.takeMeal(portion)) {
					e.needs.hunger = Math.max(0, e.needs.hunger - 62);
			...
			case "deliver": {
				...
				const wage = stored * this.market.price(load.good) * .35;
				e.wealth += wage;
				const household = this.householdFor(e);
			...
			case "gather_ore":
				...
				if (SimplePathfinder.distance(e.x, e.y, e.targetX, e.targetY) < 1) {
					...
					if (oreTile?.resourceType && this.pickUpLoad(e, tileMap, e.targetX, e.targetY, oreTile.resourceType)) e.aiState = "deliver";
					e.targetX = null;
					e.targetY = null;
				} else {
					const pos = SimplePathfinder.getStepTowards(e.x, e.y, e.targetX, e.targetY, tileMap, speed);
	...
	*/
	autoArmSoldier(e) {
		...
		for (const tier of EQUIPMENT_TIERS_BY_RANK) {
			...
			if (tier !== "primitive") {
				if (!city) continue;
				if (!Object.entries(cost).every(([good, amount]) => city.stock.get(good) >= amount)) continue;
				for (const [good, amount] of Object.entries(cost)) {
					const used = city.stock.take(good, amount);
	...
	*/
	tickGeopolitics() {
		...
		for (let i = 0; i < kingdoms.length; i++) for (let j = i + 1; j < kingdoms.length; j++) {
			...
			if (relation > -12 && !(proximity > .82 && relation <= -4)) continue;
			...
			const borderAmbition = aggressor.culture.militarism * .3 + aggressor.culture.expansionism * .28 + defender.culture.militarism * .2 + defender.culture.expansionism * .18;
			const powerRatio = aggressor.computePower() / Math.max(1, defender.computePower());
			const confidence = Math.min(2, powerRatio);
			const hostility = Math.max(.01, -relation / 120);
			if (rng.chance(.1 * aggression * confidence * (1 + hostility + proximity * borderAmbition * .8))) {
	...
	musterArmies() {
		for (const kingdom of this.kingdoms.values()) {
			...
			for (const cityId of kingdom.cityIds) {
				...
				let soldiersNow = 0;
				for (const e of this.entities) if (e.cityId === cityId && e.hp > 0 && e.profession === "soldier") soldiersNow++;
				const levy = Math.max(2, Math.round(city.population * .12));
				const need = Math.max(0, levy - soldiersNow);
				if (need <= 0) continue;
				const food = city.stock.get("food");
				const civilians = this.entities.filter((e) => e.cityId === cityId && e.hp > 0 && !e.isChild && e.profession !== "soldier" && e.profession !== "king" && e.profession !== "none");
				const nonFood = civilians.filter((e) => e.profession !== "farmer" && e.profession !== "woodcutter");
				const pool = nonFood.length > 0 && food >= city.population * 1.2 ? nonFood : food >= city.population * 3 ? civilians : [];
	...
	tickFamilies(tileMap) {
		...
		for (const [cityId, residents] of byCity) {
			...
			for (const parent of residents) {
				...
				let chance = .7 * city.prosperity + Math.min(.32, foodPerHead / 20);
				if (parent.species === SpeciesType.STONEKIN) {
					if (city.stock.get("stone") < 4) continue;
					city.ledger.recordConsumed("stone", city.stock.take("stone", 4));
					chance *= .9;
	...
	*/
	tickWildlife(tileMap) {
		...
		for (const e of this.entities) {
			...
			if (!list) {
				list = [];
				counts.set(e.species, list);
			}
			list.push(e);
		}
		const area = tileMap.width * tileMap.height;
		const isPredator = (s) => s === SpeciesType.WOLF || s === SpeciesType.BEAR || s === SpeciesType.DRAGON || s === SpeciesType.EAGLE;
		for (const [species, members] of counts) {
			const cap = isPredator(species) ? Math.max(2, Math.round(area / 900 * .22)) : Math.max(6, Math.round(area / 900 * 1.8));
			if (members.length >= cap) continue;
			const breeding = members.filter((m) => m.age >= 1 && m.hp > m.maxHp * .5);
			...
			for (let i = 0; i < litters; i++) {
				...
				if (!rng.chance(isPredator(species) ? .35 : .6)) continue;
...
//#endregion
//#region scratch/test_technology.ts
/**
...
console.log("=== PHASE J: TECHNOLOGY CHECKS ===\n");
{
	...
	check("oil means nothing to a stone-age realm", strategicWeight(stoneAge, "oil") === 0, `${strategicWeight(stoneAge, "oil")}`);
	...
	check("industrialisation makes oil matter", strategicWeight(industrial, "oil") > 0, `${strategicWeight(industrial, "oil")}`);
	const modern = new ResearchState();
	modern.complete("electricity");
	check("electricity is what finally makes uranium matter", strategicWeight(modern, "uranium") > 0, `${strategicWeight(modern, "uranium")}`);
}
{
...
}
{
	...
	check("tin outranks copper in the bronze recipe", strategicWeight(after, "tin") > strategicWeight(after, "copper"), `tin ${strategicWeight(after, "tin")} vs copper ${strategicWeight(after, "copper")}`);
...
}
{
	...
	check("the list is sorted strongest first", ranked.every((g, i) => i === 0 || ranked[i - 1].weight >= g.weight), ranked.map((g) => `${g.good}:${g.weight}`).join(" "));
}
{
	const r = new ResearchState();
	r.complete("mining");
	...
	check("a realm with nothing cannot operate what it knows", industrialBarren.capacity === 0, `${industrialBarren.capacity}`);
	check("and it can say exactly what it is missing", industrialBarren.missingGoods.length > 0 && industrialBarren.missingBuildings.length > 0, `goods=${industrialBarren.missingGoods} buildings=${industrialBarren.missingBuildings}`);
	const equipped = technologyCapacity(r, () => true, () => true);
	const industrialEquipped = equipped.find((c) => c.techId === "industrialization");
	check("a fully supplied realm operates its technology completely", industrialEquipped.capacity === 1, `${industrialEquipped.capacity}`);
	check("an unusable industrial tree does not make you an industrial power", operatingEra(r, barren) !== "industrial", operatingEra(r, barren));
	check("a supplied industrial tree does make you an industrial power", operatingEra(r, equipped) === "industrial", operatingEra(r, equipped));
}
{
	const r = new ResearchState();
	r.complete("industrialization");
	const cap = technologyCapacity(r, () => true, () => false).find((c) => c.techId === "industrialization").capacity;
...
check("every non-primitive tier is gated by a technology", EQUIPMENT_TIERS_BY_RANK.filter((t) => t !== "primitive").every((t) => EQUIPMENT_TECH[t] !== null));
check("firearms exist as real equipment once gunpowder is known", (WEAPON_TIERS.gunpowder ?? []).length > 0);
{
	...
	check("a realm that knows rifles but makes nothing fields primitive arms", !!bare && (WEAPON_TIERS.primitive ?? []).some((w) => w.name === bare.name), bare?.name ?? "unarmed");
	...
	check("the same realm with a real arsenal fields industrial arms", !!armed && (WEAPON_TIERS.industrial ?? []).some((w) => w.name === armed.name), armed?.name ?? "unarmed");
	check("arming a soldier physically consumed materials", city.stock.get("steel") < steelBefore, `${steelBefore} -> ${city.stock.get("steel")}`);
}
{
	...
	if (kingdom) {
		...
		check("the realm reports what it can actually operate", kingdom.techCapabilities.length > 0 && kingdom.operatingEra !== void 0, `${kingdom.techCapabilities.length} caps, era ${kingdom.operatingEra}`);
		check("a realm with no mines cannot operate steam power fully", kingdom.technologicalCapacity() < 1, `${kingdom.technologicalCapacity().toFixed(2)}`);
	} else check("a kingdom formed for the demand test", false, "no kingdom");
}
